...
private:
	...
	void setupDebugMessenger() {
		...
		std::cout << "Successfully setup validation layers" << std::endl;
	}
	...
	void pickPhysicalDevice() {
		...
		if (deviceCount == 0) {
			...
			std::cout << "found " << deviceCount << " vulkan supported GPU" << std::endl;
		}
		...
		if (physicalDevice == VK_NULL_HANDLE) {
			...
			std::cout << "found a suitable GPU" << std::endl;
		}
	}
	...
	void createCommandBuffers() {
		...
		for (size_t i = 0; i < commandBuffers.size(); i++) {
			...
			if (vkEndCommandBuffer(commandBuffers[i]) != VK_SUCCESS) {
				throw std::runtime_error("faieled to record command buffer");
			}
		}
	...
	VkPresentModeKHR chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& avaialablePresentModes) {
		...
		for (const auto& avaialablePresentMode : avaialablePresentModes) {
			if (avaialablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
